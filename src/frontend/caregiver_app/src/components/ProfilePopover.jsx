...
export const ProfilePopover = () => {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-64 bg-surface dark:bg-ink border border-border dark:border-ink-soft/40 rounded-card shadow-card p-3.5 z-50 font-sans animate-in fade-in zoom-in-95 duration-150">
          <div className="flex items-center gap-3 pb-3 border-b border-border/60 dark:border-ink-soft/30 mb-2.5">
            <div className="w-10 h-10 rounded-full bg-terracotta text-cream flex items-center justify-center font-bold text-sm shrink-0 shadow-xs">
              {caregiverInitials}
            </div>
            <div className="min-w-0 flex-1">
              <div className="font-bold text-sm text-ink dark:text-cream truncate">
                {caregiver.name}
              </div>
              <div className="text-xs text-ink-soft dark:text-cream/70 truncate">
          ...
          <button
            ...
            className="w-full text-left px-3 py-2 rounded-lg text-xs sm:text-sm text-terracotta hover:bg-cream dark:hover:bg-ink-soft/20 font-medium flex items-center gap-2 transition-colors"
          >
